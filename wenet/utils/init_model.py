...
from wenet.transducer.s2c_model import S2CModel
...
def init_s2c_model(configs):
    ...
    global_cmvn = None
    if len(input_dim) == 2:
        from wenet.transducer.embedding import S2cEmbedding
        embed = S2cEmbedding(input_dim=input_dim, **configs['embedding_conf'])
        input_dim = configs['embedding_conf']['embed_size'] * 2
    if encoder_type == 'conformer':
        encoder = ConformerEncoder(input_dim,
                                   global_cmvn=global_cmvn,
                                   **configs['encoder_conf'])
    ...
    if 'predictor' in configs:
        ...
        configs['joint_conf']['pred_output_size'] = configs['predictor_conf'][
            'output_size']
        joint = TransducerJoint(vocab_size, **configs['joint_conf'])
        model = S2CModel(vocab_size=vocab_size,
                           blank=0,
                           embed=embed,
                           predictor=predictor,
                           encoder=encoder,
