...
from wenet.transducer.search.prefix_beam_search import S2CPrefixBeamSearch
...
class S2CModel(ASRModel):
    ...
    def __init__(
        ...
        super().__init__(vocab_size, encoder, attention_decoder,None,
                         0.5, ignore_id, reverse_weight, lsm_weight,
    ...
    def forward(
        ...
        assert (txt_input.shape[0] == syl_input.shape[0] == labels.shape[0] == input_length.shape[0] == output_length.shape[0]), (
                txt_input.shape, syl_input.shape,labels.shape,
                input_length.shape, output_length.shape)
        # Embedding
        embedded = self.embed(txt_input, syl_input, )
        # Encoder
        encoder_out, encoder_mask = self.encoder(embedded, input_length)
        encoder_out_lens = encoder_mask.squeeze(1).sum(1)
        # predictor
        ys_in_pad = add_blank(labels, self.blank, self.ignore_id)
        predictor_out = self.predictor(ys_in_pad)
        # joint
        joint_out = self.joint(encoder_out, predictor_out)
        # NOTE(Mddct): some loss implementation require pad valid is zero
        # torch.int32 rnnt_loss required
        rnnt_text = labels.to(torch.int64)
        rnnt_text = torch.where(rnnt_text == self.ignore_id, 0,
                                rnnt_text).to(torch.int32)
        rnnt_text_lengths = output_length.to(torch.int32)
        ...
        if self.attention_decoder_weight != 0.0 and self.attention_decoder is not None:
            loss_att, _ = self._calc_att_loss(encoder_out, encoder_mask, labels,
                                              output_length)
    ...
    @torch.jit.export
    def ctc_activation(self, xs:torch.Tensor)-> Optional[torch.Tensor]:
        return None
    def init_bs(self):
        if self.bs is None:
            self.bs = S2CPrefixBeamSearch(self.embed, self.encoder, self.predictor,
                                       self.joint, self.blank)
