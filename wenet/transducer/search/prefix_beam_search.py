...
class S2CPrefixBeamSearch():

    def __init__(self, embedding, encoder, predictor, joint, blank):
    ...
    def prefix_beam_search(self,
        ...
        """prefix beam search
           also see wenet.transducer.transducer.beam_search
        """
        assert txt_input.shape[0] == input_lengths.shape[0]
        assert decoding_chunk_size != 0
        device = txt_input.device
        batch_size = txt_input.shape[0]
        assert batch_size == 1

        embedded = self.embed(txt_input, syl_input)
        # 1. Encoder
        encoder_out, _ = self.encoder(
            embedded, input_lengths, decoding_chunk_size,
            num_decoding_left_chunks)  # (B, maxlen, encoder_dim)
        maxlen = encoder_out.size(1)
