...
from wenet.language_model.dataset import Dataset_s2c
from wenet.utils.checkpoint import load_checkpoint
from wenet.utils.file_utils import read_symbol_table, read_non_lang_symbols
from wenet.utils.config import override_config
from wenet.utils.init_model import init_s2c_model
...
def get_args():
    ...
    parser.add_argument('--syl_table',
                        required=True,
                        help='model syllable table for training')
...
def main():
    ...
    syl_table = read_symbol_table(args.syl_table)
    ...
    test_dataset = Dataset_s2c(args.data_type,
                           args.test_data,
                           symbol_table,
                           syl_table,
                           test_conf,
    ...
    with torch.no_grad(), open(args.result_file, 'w') as fout:
        for batch_idx, batch in enumerate(test_data_loader):
            net_input,num_utts = model.net_input_process(**batch, device=device)
            keys = batch['key']
            txt_input = net_input["txt_input"]
            syl_input = net_input["syl_input"]
            input_length = net_input["input_length"]
            if args.mode == 'rnnt_beam_search':
